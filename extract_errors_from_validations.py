...
def testing_results(succ_csvs, unsucc_csvs, n_lags, prediction_model, promp_models):
    
    for csv in succ_csvs:
        print "skip the succ_csv"
        continue
    ...
    for csv in unsucc_csvs:
        ...
        for feature in range(n_features):
            ...
            means, stds = promp_models[feature].plot_prior(b_regression=False, linewidth_mean=3, b_dataset=True)
            ...
            for idx in range(len(stamp)):
                ...
                std = stds[ix]
                if status(e = e[idx], mean = mean, std = std, c = 2.0) == "anomalies":
...
if __name__=="__main__":
    ...
    testing_results(succ_csvs = succ_csvs,
